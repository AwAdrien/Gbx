...
def Chunk00B(bp):
    bp.customList(
        [(lambda x: x.uint32, "time"), (lambda x: x.uint32, "stuntScore")],
        "checkpoints",
    )
...
def Chunk00E(bp):
    bp.lookbackString("uid", is_local=False)
...
def Chunk019(bp):
    ...
    if eventsDuration == 0:
        return
    bp.uint32("u1")
    bp.customList([(lambda x: x.lookbackString, "controlNames")], "controlNameList")
