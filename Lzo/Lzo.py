...
class LZO:
    ...
    def __init__(self):
        ...
        if osname == "nt":
            ...
            raise Exception(
                f"Your system cannot load the LZO libs. Required: Windows/Linux, given: {osname}"
            )

        self.__lzo1x_lib_path = path.join(
        ...
        self.__lzo1x_lib.lzo1x_decompress_safe.argtypes = [
            c_char_p,
            c_uint32,
            c_char_p,
            POINTER(c_uint32),
        ...
        self.__lzo1x_lib.lzo1x_999_compress.argtypes = [
            c_char_p,
            c_uint32,
            c_char_p,
            POINTER(c_uint32),
    ...
    def decompress(self, data, uncompressed_size) -> bytes:
        import binascii
        decomp =  self.__lzo1x_decompress_safe(data, uncompressed_size)
        # print(f"decompress: {binascii.hexlify(decomp[:1000])}")
        return decomp

    def compress(self, data) -> bytes:
        import binascii
        # print(f"compress:   {binascii.hexlify(data[:1000])}")
    ...
    def __lzo1x_decompress_safe(self, data, uncompressed_size):
        if not isinstance(data, bytes):
            try:
                ...
                logging.error(f"Could not turn data into type bytes: {e}")
                raise Exception
        if not isinstance(uncompressed_size, int):
            logging.error(
                f"uncompressed_size must be of data type int. {type(uncompressed_size)} was given"
            )
            raise Exception
        ...
        try:
            if self.__lzo1x_lib.lzo1x_decompress_safe(
                data, compressed_size, out_buffer, byref(bytes_written)
            ):
                raise Exception
    ...
    def __lzo1x_999_compress(self, data):
        ...
        try:
            if (
                self.__lzo1x_lib.lzo1x_999_compress(
                    data,
                    uncompressed_size,
                    out_buffer,
                    byref(bytes_written),
